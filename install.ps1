...
if ($LASTEXITCODE -ne 0) { Write-Host "[X] pnpm build failed." -ForegroundColor Red; exit 1 }
...
cmd.exe /c $npmCmd
if ($LASTEXITCODE -ne 0) { Write-Host "[X] npm install globally failed." -ForegroundColor Red; exit 1 }
